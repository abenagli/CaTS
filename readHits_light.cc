...
int main(int argc, char** argv)
{
  if( argc < 3 )
  {
    G4cout << "Program requires at least 2 arguments: input file list, base name of output file, maxEntries (-1), doByParticle (false), doByParticleType (false), doByProcess (false), HCMap (false)" << G4endl;
    exit(1);
  }
  ...
  G4bool doByParticle = false;
  G4bool doByParticleType = false;
  G4bool doByProcess = false;
  if( argc > 3 ) maxEntries = G4int(atoi(argv[3]));
  if( argc > 4 ) doByParticle = G4bool(atoi(argv[4]));
  if( argc > 5 ) doByParticleType = G4bool(atoi(argv[5]));
  if( argc > 6 ) doByProcess = G4bool(atoi(argv[6]));
  if( argc > 7 ) doHCMap = G4bool(atoi(argv[7]));
  ...
  while(1)
  {
    ...
    Tevt -> SetBranchStatus("*",0);
    Tevt -> SetBranchStatus("TotDepAndEscAndLosEnergy",1);
    Tevt -> SetBranchStatus("TotDepAndEscEnergy",1);
    Tevt -> SetBranchStatus("TotDepEnergy",1);
    Tevt -> SetBranchStatus("TotObsEnergy",1);
    Tevt -> SetBranchStatus("TotNCeren",1);
    Tevt -> SetBranchStatus("m_EdepAndEscAndLos",1);
    Tevt -> SetBranchStatus("m_EdepAndEsc",1);
    Tevt -> SetBranchStatus("m_Edep",1);
    Tevt -> SetBranchStatus("m_Eobs",1);
    Tevt -> SetBranchStatus("m_NCeren",1);
    Tevt -> SetBranchStatus("m_Eobs_byTime",1);
    if( doByParticle )     Tevt -> SetBranchStatus("m_Eobs_byParticleAndTime",1);
    if( doByParticleType ) Tevt -> SetBranchStatus("m_Eobs_byParticleTypeAndTime",1);
    if( doByProcess )      Tevt -> SetBranchStatus("processAndTimeMult",1);
    if( doByProcess )      Tevt -> SetBranchStatus("processAndTimeMap",1);
    if( doHCMap )          Tevt -> SetBranchStatus("HCMap",1);
    Tevt -> SetBranchStatus("m_particleMult",1);
    Tevt -> SetBranchStatus("m_particleHist",1);
    ...
    std::map<G4String,std::map<G4String,TH1F*> > h_n_byParticle;
    std::map<G4String,std::map<G4String,TH1F*> > h_n_byParticleType;
    ...
    for(unsigned int volIt = 0; volIt < volumeList->size(); ++volIt)
    {
      ...
      if( doByParticle )
      {
        for(unsigned int partIt = 0; partIt < particleList->size(); ++partIt)
        {
          ...
          h_n_byParticle[volName][partName] = new TH1F(Form("h_n_%s_%s",volName.c_str(),partName.c_str()),"",10000,0.,10000.);
          h_Ekin_byParticle[volName][partName] = new TH1F(Form("h_Ekin_%s_%s",volName.c_str(),partName.c_str()),"",100000,0.,1000.);
        }
      }
      ...
      if( doByParticleType )
      {
        for(unsigned int partTypeIt = 0; partTypeIt < particleTypeList->size(); ++partTypeIt)
        {
          ...
          h_n_byParticleType[volName][partTypeName] = new TH1F(Form("h_n_%s_%s",volName.c_str(),partTypeName.c_str()),"",10000,0.,10000.);
        ...
        for(unsigned int timeSliceTypeIt = 0; timeSliceTypeIt < timeSliceTypes.size(); ++timeSliceTypeIt)
        {
          ...
          if( doByProcess )
          {
            for(unsigned int procIt = 0; procIt < processList->size(); ++procIt)
            {
              ...
              Ekin_byProcessAndTime[volName][timeSliceName][procName] = new TProfile(Form("Ekin%s_%s_%s",volName.c_str(),procName.c_str(),timeSliceName.c_str()),"Ekin",nBins,min,max,"s");
            }
          }
        }
      }
    ...
    for(G4int entry = 0; entry < nEvents; ++entry)
    {
      ...
      std::map<G4String,std::map<G4String,std::map<G4int,std::map<G4String,G4float> > > >* m_Eobs_byParticleAndTime;
      if( doByParticle )
      {
        m_Eobs_byParticleAndTime = event->GetEobsByParticleAndTimeMap(); // map<Detector,map<timeSliceType,map<timeSlice,map<particle,energy> > > >
      }
      
      std::map<G4String,std::map<G4String,std::map<G4int,std::map<G4String,G4float> > > >* m_Eobs_byParticleTypeAndTime;
      if( doByParticleType )
      {
        m_Eobs_byParticleTypeAndTime = event->GetEobsByParticleTypeAndTimeMap(); // map<Detector,map<timeSliceType,map<timeSlice,map<particle,energy> > >
      }
      
      std::map<G4String,std::map<G4String,std::map<G4int,std::map<G4String,G4int> > > >* m_mult_byProcessAndTime;
      std::map<G4String,std::map<G4String,std::map<G4int,std::map<G4String,std::map<G4String,products> > > > >* m_products_byProcessAndTime;
      if( doByProcess )
      {
        m_mult_byProcessAndTime = event->GetProcessAndTimeMult(); // map<Detector,map<timeSliceType,map<timeSlice,map<processes,mult> > > >
        m_products_byProcessAndTime = event->GetProcessAndTimeMap(); // map<Detector,map<timeSliceType,map<timeSlice,map<process,map< particle,products> > > > >
      }
      
      std::map<G4String,std::map<G4String,std::map<G4ThreeVector,std::vector<G4VHit*> > > >* HCMap;
      if( doHCMap )
      {
        HCMap = event->GetHCMap();
      }
      
      std::map<G4String,std::map<G4String,int> >* m_particleMult = event->GetParticleMult();   // map<Detector,map<particle,TH1F*> >
      ...
      for(unsigned int volIt = 0; volIt < volumeList->size()-1; ++volIt)
      {
        ...
        if( doByParticle )
        {
          for(unsigned int partIt = 0; partIt < particleList->size(); ++partIt)
          {
            ...
            h_n_byParticle[volName][partName] -> Fill((*m_particleMult)[volName][partName]);
            sumNPart[partName] += (*m_particleMult)[volName][partName];
      ...
      if( doByParticle )
      {
        ...
        {
          std::string partName = particleList->at(partIt);
          h_n_byParticle["AllVol"][partName] -> Fill(sumNPart[partName]);
        }
      }
