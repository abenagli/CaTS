...
$PHYSList    = $User_Preferences{"PHYSList"};
...
while(<LIST>)
{
  ...
  for($iJob = $Nfirst; $iJob < $Nfirst+$Njobs; ++$iJob)
  {
    ...
    if( $ISLxplus )
    {
      print JOBSH "source ~/bin/myQt4.8.sh\n";
      print JOBSH "source /afs/cern.ch/sw/lcg/external/gcc/4.9/x86_64-slc6-gcc49-opt/setup.sh\n";
      print JOBSH ". /afs/cern.ch/sw/lcg/external/geant4/10.1.p01/x86_64-slc6-gcc49-opt/CMake-setup.sh\n";
      print JOBSH "export PHYSLIST=".$PHYSList."\n";
