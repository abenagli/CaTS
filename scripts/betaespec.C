//**********************************************************************************************
//
// Script to display beta spectrum of charged particles in an em shower
// To run it:  
// TFile *f = TFile::Open("../../CaTS-build/data/PbWO_FTFPBERT_e-_100.0GeV_analysis.root");
// .L betaespec.C+
// betaespec(f);
//
// Author: Hans Wenzel
...
TCanvas* betaespec( TFile* f)
{
 ...
 TH1F * halpha = (TH1F*) f->Get("Particles/alpha");
  ...
  h->GetYaxis()->SetTitle("Nr of charged tracks");
  hs->Draw("same");
  TLegend *legend = new TLegend(0.15,0.6,0.3,0.89);
  legend->AddEntry(h,"nuclear fragments","f");
  legend->AddEntry(h1,"e^{+}","f");
  legend->AddEntry(h2,"e^{-}","f");
  legend->AddEntry(h5,"p","f");
  legend->AddEntry(hmum,"#mu^{-}","f");
  legend->AddEntry(hmup,"#mu^{+}","f");
  legend->AddEntry(halpha,"#alpha","f");
  legend->AddEntry(h4,"#pi^{+}","f");
