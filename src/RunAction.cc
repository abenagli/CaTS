...
void RunAction::BeginOfRunAction(const G4Run* aRun) {
    ...
    rh->SetParticleEnergy(pvG->GetParticleEnergy());
    rh->SetParticleTime(pvG->GetParticleTime());
    rh->SetParticlePosition(pvG->GetParticlePosition());
    ...
    rh->SetParticleListCeren(cSD->GetParticleListCeren());
    rh->SetParticleListShort(cSD->GetParticleListShort());
    rh->SetTimeSliceLow(cSD->GetTimeSliceLow());
    rh->SetMinTimeLow(cSD->GetMinTimeLow());
    rh->SetMaxTimeLow(cSD->GetMaxTimeLow());
    rh->SetTimeSliceMed(cSD->GetTimeSliceMed());
    rh->SetMinTimeMed(cSD->GetMinTimeMed());
    rh->SetMaxTimeMed(cSD->GetMaxTimeMed());
    rh->SetTimeSliceHig(cSD->GetTimeSliceHig());
